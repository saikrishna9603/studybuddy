...
def init_db(app):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        # ─────────────────────────────────────────────────────────────────
        # Interview & Placement Assistant Tables
        # ─────────────────────────────────────────────────────────────────
        conn.execute(
            """
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_interview_sessions_email ON interview_sessions(user_email)"
        )
        conn.execute(
            """
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_evaluation_results_answer ON evaluation_results(answer_id)"
        )
        conn.execute(
            """
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_roadmaps_user ON roadmaps(user_email)"
        )
        conn.commit()
...
def create_interview_session(db_path, user_email, resume_id, company, role, difficulty=3):
    """Create a new interview session."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
            """
            INSERT INTO interview_sessions
            (user_email, resume_id, company, role, difficulty)
            VALUES (?, ?, ?, ?, ?)
            """,
            (user_email, resume_id, company, role, difficulty),
        )
...
def get_interview_session(db_path, session_id):
...
def list_user_interview_sessions(db_path, user_email):
...
def update_interview_session_score(db_path, session_id, total_score, status='completed'):
    """Update interview session with final score."""
    with get_connection(db_path) as conn:
        conn.execute(
            """
            UPDATE interview_sessions
            SET total_score = ?, status = ?, completed_at = datetime('now')
            WHERE id = ?
            """,
...
def create_interview_question(db_path, session_id, question_type, question_text, difficulty, expected_keywords):
    """Create an interview question for a session."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
            """
            INSERT INTO interview_questions
            (session_id, question_type, question_text, difficulty, expected_keywords)
            VALUES (?, ?, ?, ?, ?)
            """,
            (session_id, question_type, question_text, difficulty, expected_keywords),
        )
...
def list_interview_questions(db_path, session_id):
    """List all questions for an interview session."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
            """
...
def create_user_answer(db_path, session_id, question_id, user_email, answer_text, time_taken=None):
    """Save a user's answer to an interview question."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
            """
            INSERT INTO user_answers
            (session_id, question_id, user_email, answer_text, time_taken)
            VALUES (?, ?, ?, ?, ?)
            """,
            (session_id, question_id, user_email, answer_text, time_taken),
        )
...
def get_user_answer(db_path, answer_id):
    """Retrieve a user's answer."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
...
def list_session_answers(db_path, session_id):
    """List all answers submitted in a session."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
            """
...
def create_evaluation_result(db_path, answer_id, score_correctness, score_clarity, 
                            score_depth, score_communication, strengths, weaknesses, 
                            model_answer, tips):
    """Save evaluation results for a user's answer."""
    overall_score = (score_correctness + score_clarity + score_depth + score_communication) / 4.0
    
    with get_connection(db_path) as conn:
        cur = conn.execute(
            """
            INSERT INTO evaluation_results
            (answer_id, score_correctness, score_clarity, score_depth, score_communication,
             overall_score, strengths, weaknesses, model_answer, tips)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (answer_id, score_correctness, score_clarity, score_depth, score_communication,
             overall_score, strengths, weaknesses, model_answer, tips),
...
def get_evaluation_result(db_path, answer_id):
    """Retrieve evaluation results for an answer."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
            "SELECT * FROM evaluation_results WHERE answer_id = ?",
            (answer_id,),
        )
        row = cur.fetchone()
        return dict(row) if row else None
...
def list_session_evaluations(db_path, session_id):
    """List all evaluations for answers in a session."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
            """
            SELECT er.* FROM evaluation_results er
            JOIN user_answers ua ON er.answer_id = ua.id
            WHERE ua.session_id = ?
...
def create_roadmap(db_path, user_email, resume_id, company, weak_areas, study_plan, resources):
    """Create a personalized roadmap for interview preparation."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
            """
            INSERT INTO roadmaps
            (user_email, resume_id, company, weak_areas, study_plan, resources)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (user_email, resume_id, company, weak_areas, study_plan, resources),
        )
...
def get_roadmap(db_path, roadmap_id):
    """Retrieve a roadmap."""
    with get_connection(db_path) as conn:
        cur = conn.execute(
...
def list_user_roadmaps(db_path, user_email):
...
def update_roadmap_progress(db_path, roadmap_id, progress):
